function prompt_login --description "display user name for the prompt"
	...
	if test "$USER" = "liushengyu"
		set DISPLAY_USER lsy
    else
        set DISPLAY_USER "$USER"
	end
	
    echo -n -s (set_color $fish_color_user) "$DISPLAY_USER" (set_color normal) @ (set_color $color_host) ds-dev (set_color normal)
end
